...
public final class BurnCommand {
    ...
    private static int addFire(CommandContext<CommandSource> context, Collection<? extends Entity> targets, int time, boolean preferTicks){
        ...
        if (targets.size() == 1) {
            context.getSource().sendFeedback(new TranslationTextComponent("commands.burn.success.single", targets.iterator().next().getDisplayName(), time, timeAmountFormat), true);
        } else {
            context.getSource().sendFeedback(new TranslationTextComponent("commands.burn.success.multiple", targets.size(), time, timeAmountFormat), true);
        }
