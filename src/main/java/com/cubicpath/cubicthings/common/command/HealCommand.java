...
public final class HealCommand {
    ...
    public static void register(CommandDispatcher<CommandSource> dispatcher){
        dispatcher.register(Commands.literal(COMMAND_NAME).requires((context) -> {
            ...
            return heal(context, ImmutableList.of(context.getSource().assertIsEntity()), null);
    ...
    private static int heal(CommandContext<CommandSource> context, Collection<? extends Entity> targets, @Nullable Float amount){
        ...
        if (targets.size() == 1) {
            context.getSource().sendFeedback(new TranslationTextComponent("commands.heal.success.single", targets.iterator().next().getDisplayName(), amount), true);
        } else {
            context.getSource().sendFeedback(new TranslationTextComponent("commands.heal.success.multiple", targets.size(), amount), true);
        }
