...
public class ScannerItem extends Item implements INamedContainerProvider, IDefaultNBTHolder {
    ...
    protected static class ScanContext {
        ...
        protected ScanContext(World world, BlockPos blockPos, @Nullable Float scanVolume, float scanPitch){
            this.world = world;
    ...
    public boolean onEntitySwing(ItemStack stack, LivingEntity entity) {
        if (entity.isCrouching()) {
            if (entity instanceof ServerPlayerEntity) {
                NetworkHooks.openGui((ServerPlayerEntity) entity, this, (packetBuffer) -> packetBuffer.writeItem(stack));
            }
            return true;
        }
    ...
    public ActionResult<ItemStack> use(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        Boolean[] scanResults;
        ActionResult<ItemStack> interactionResultHolder;
        ...
        interactionResultHolder = Arrays.stream(scanResults).anyMatch((scanResult) -> scanResult) ? ActionResult.consume(itemStack) : ActionResult.pass(itemStack);
        // If any of the results are true, consume; else pass.
        if (playerIn instanceof ServerPlayerEntity && interactionResultHolder.getResult() == ActionResultType.CONSUME && !playerIn.isCreative()) {
            itemStack.hurt(1, playerIn.getRandom(), (ServerPlayerEntity)playerIn);
        }
    ...
    public static boolean scanForEntities(World world, LivingEntity scanner, BlockPos pos, EntityType<?>[] entityTypes, @Nullable Float volume, float pitch) {
        ...
        if (Arrays.stream(entityTypes).anyMatch((entityType) -> {
            entitiesWithinAABB.addAll(world.getEntities(entityType, new AxisAlignedBB(pos), (entity) -> !entity.equals(scanner)));
